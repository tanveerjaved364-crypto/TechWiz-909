...
def generate(employee):
    reqs=rows("SELECT * FROM requirements WHERE role=? ORDER BY mandatory DESC, id",(employee['role'],))
    items=[]
    for r in reqs:
...
def validate(plan, employee):
    approved={r['id']:r for r in rows("SELECT r.* FROM requirements r JOIN documents d ON d.id=r.document_id WHERE r.role=? AND d.status='Active'",(employee['role'],))}
...
def upload():
    ...
    title=request.form.get('title') or secure_filename(f.filename); version=request.form.get('version','1.0'); effective=request.form.get('effective_date',str(date.today())); category=request.form.get('category','Uploaded'); department=request.form.get('department','Corporate')
...
def approve(doc_id): execute("UPDATE documents SET status='Active' WHERE id=?",(doc_id,)); return jsonify(message='Document approved')
